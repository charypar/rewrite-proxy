...
func TestReadRSAPrivateKey(t *testing.T) {
	...
	primes0 := big.NewInt(0)
	primes0.SetBytes([]byte{0xdf, 0x34, 0xad, 0xc6, 0x77, 0x1c, 0x3a, 0xbb, 0xf4, 0x2e, 0xd7, 0xe3, 0xfe, 0x85, 0x72, 0x7d, 0x8c, 0x44, 0xf7, 0xa6, 0xa8, 0xd5, 0xcd, 0x52, 0x29, 0x94, 0xce, 0x7, 0x8a, 0xf6, 0x3, 0x79, 0x75, 0x44, 0x98, 0xe8, 0xc, 0x37, 0xe6, 0x3e, 0x6a, 0xac, 0x59, 0xd, 0xc9, 0xd8, 0xb1, 0xfe, 0x66, 0xea, 0xae, 0xe6, 0x5b, 0xc0, 0xb5, 0x42, 0x9f, 0x14, 0xb9, 0xdf, 0x33, 0x29, 0x75, 0xfc, 0x98, 0x2e, 0x7a, 0xff, 0xfb, 0xd6, 0xbe, 0x2c, 0xcb, 0x90, 0xdb, 0xb3, 0x2b, 0x5d, 0xd3, 0x8e, 0x3, 0xc1, 0xf6, 0x4, 0xd6, 0x67, 0x27, 0x6f, 0xf7, 0xe7, 0x86, 0x97, 0x17, 0x8c, 0x73, 0x26, 0xaf, 0xe2, 0x8, 0xca, 0xfb, 0xae, 0x55, 0xad, 0xcb, 0x30, 0xc8, 0x93, 0x4f, 0xa6, 0x35, 0x96, 0x77, 0xee, 0x20, 0x95, 0xc0, 0xd4, 0xf5, 0x7d, 0xc7, 0x78, 0x18, 0xbb, 0x12, 0xc0, 0x5c, 0x81})

	primes1 := big.NewInt(0)
	primes1.SetBytes([]byte{0xd2, 0x9c, 0x72, 0x46, 0xff, 0x84, 0xc0, 0x4f, 0x1c, 0xbc, 0x12, 0xa, 0x86, 0x1f, 0x69, 0x92, 0xbc, 0x62, 0xd4, 0x33, 0xb7, 0x55, 0x82, 0xf7, 0xf5, 0x92, 0xf1, 0x8d, 0xb5, 0xfa, 0xdf, 0x4f, 0xe1, 0xca, 0xd6, 0xce, 0x6b, 0x5, 0x1d, 0xe7, 0x2, 0x1f, 0xd3, 0xc9, 0x89, 0xed, 0xf2, 0x43, 0xa7, 0x2a, 0x69, 0x6c, 0x9b, 0xd7, 0xaa, 0x43, 0xa0, 0x81, 0xe7, 0xee, 0x1c, 0x45, 0xa0, 0x37, 0x39, 0x8b, 0x48, 0x42, 0x5e, 0xa3, 0xaf, 0x10, 0xde, 0xc6, 0x5a, 0xc8, 0x82, 0x7, 0xee, 0xfd, 0xca, 0xc6, 0x41, 0x15, 0xd6, 0x8f, 0x90, 0xed, 0x36, 0x15, 0xfe, 0x14, 0x6, 0xf5, 0xac, 0xbc, 0xe4, 0x4a, 0xa5, 0x86, 0xac, 0xc5, 0x9b, 0xee, 0x9a, 0x78, 0x93, 0xf, 0x52, 0xe7, 0xb6, 0xe, 0xb7, 0x5e, 0x7b, 0x41, 0x74, 0x7c, 0x66, 0xcd, 0x6, 0x5a, 0x2a, 0xf3, 0x7e, 0x23, 0xa9, 0xcb})

	primes := []*big.Int{primes0, primes1}
...
-----END RSA PRIVATE KEY-----`),
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if err == nil && (!reflect.DeepEqual(got.D, tt.want.D) || !reflect.DeepEqual(got.Primes, tt.want.Primes)) {
				t.Errorf("ReadRSAPrivateKey() got = %v,\n want = %v,\n\n Gexponent = %#v,\n Wexponent = %#v,\n\n Gprimes0 = %#v,\n Wprimes0 = %#v,\n\n Gprimes1 = %#v,\n Wprimes1 = %#v",
					got,
					tt.want,
					got.D.Bytes(),
					tt.want.D.Bytes(),
					got.Primes[0].Bytes(),
					tt.want.Primes[0].Bytes(),
					got.Primes[1].Bytes(),
					tt.want.Primes[1].Bytes(),
				)
			}
		})
	}
...
func TestEncryptMarshalUnmarshalDecrypt(t *testing.T) {
	priKey, err := ioutil.ReadFile("fixtures/rsa.pem")
	if err != nil {
		t.Errorf("Cannot read private key: %s", err)
	}

	pubKey, err := ioutil.ReadFile("fixtures/rsa_pub.pem")
	if err != nil {
		t.Errorf("Cannot read public key: %s", err)
	}

	privateKey, err := ReadRSAPrivateKey(priKey)
	if err != nil {
		t.Errorf("Cannot parse private key: %s", err)
	}

	publicKey, err := ReadRSAPublicKey(pubKey)
	if err != nil {
	...
	if !reflect.DeepEqual(message, decrypted) {
		t.Errorf("Decrypt(Encrypt()) = %s, want %s", string(decrypted), string(message))
	}
}
